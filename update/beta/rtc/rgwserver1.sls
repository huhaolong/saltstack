...
{{ server_name }}-config:
  file.managed:
    - name: {{ work_path }}/{{ server_name }}/config_watch.xml
    - source: salt://dev/apps_source/rgwserver/config_watch.xml
rename-{{ server_name }}-config:
  cmd.run:
    - names:
      - mv {{ work_path }}/{{ server_name }}/config_watch.xml {{ work_path }}/{{ server_name }}/config.xml
    - require:
      - file: {{ server_name }}-config
{{ server_name }}-bin:
  file.managed:
    - name: {{ work_path }}/{{ server_name }}/RGWServer1
